...
Get all possible solutions to place N which is a
...
Attributes:
    board (list): Lists representing the chessboard.
    solutions (list): Lists containing solutions.

Solutions are repr in the format [[r, c], [r, c], [r, c], [r, c]]
where `r` rep row and `c` rep column, and a
...
def get_solution(board):
    """Return lists repr of a solved chessboard."""
    solution = []
...
def xout(board, row, col):
    """X out position on the board.

    All positions where non-attacking queens can no
    longer be played labelled X-out.
